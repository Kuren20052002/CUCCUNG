          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-8 animate-fade-in-down">
              <div className="inline-flex items-center space-x-2 px-4 py-2 bg-emerald-50 rounded-full border border-emerald-100/50">
                ...
                <span className="text-[10px] font-extrabold text-emerald-800 uppercase tracking-widest leading-none">Cộng đồng Mẹ & Bé thông thái</span>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <Link
                  href="/posts"
                  className="w-full sm:w-auto px-10 py-5 bg-primary hover:bg-emerald-700 text-white font-extrabold text-sm uppercase tracking-widest rounded-[2rem] transition-all shadow-xl shadow-emerald-500/25 flex items-center justify-center gap-3 active:scale-95 group"
                >
                  Khám phá bài viết <ArrowRight className="w-5 h-5 transform group-hover:translate-x-1 transition-transform" />
                </Link>
                {/* Social proof — use CSS circles instead of external avatar images */}
                <div className="flex -space-x-4" aria-hidden="true">
                  {['bg-emerald-200', 'bg-amber-200', 'bg-pink-200', 'bg-sky-200'].map((bg, i) => (
                    <div key={i} className={`w-12 h-12 rounded-full border-4 border-white ${bg} shadow-md`} />
                  ))}
                  <div className="w-12 h-12 rounded-full border-4 border-white bg-primary flex items-center justify-center text-xs font-extrabold text-white shadow-xl">
                    +2k
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-8 pt-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest whitespace-nowrap">Nội dung đã kiểm duyệt</span>
                </div>
                <div className="flex items-center gap-2">
                  <Zap className="w-5 h-5 text-secondary" />
                  <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest whitespace-nowrap">Cập nhật hàng ngày</span>
                </div>
              </div>
            </div>
          ...
          <section className="flex-1 space-y-12">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-8">
              <div className="space-y-2">
                <h2 className="text-xs font-extrabold text-primary uppercase tracking-[0.3em]">Cẩm nang mới nhất</h2>
                <h3 className="text-4xl font-extrabold text-slate-900 tracking-tight">Dành riêng cho bạn và bé.</h3>
              </div>
              <Link href="/posts" className="text-sm font-extrabold text-slate-400 hover:text-emerald-600 uppercase tracking-widest flex items-center gap-2 group transition-all">
            ...
            <div className="bg-slate-900 rounded-[3rem] p-10 lg:p-16 relative overflow-hidden group">
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                <div className="space-y-6">
                  <h3 className="text-3xl lg:text-4xl font-extrabold text-white leading-tight underline decoration-emerald-500/30">Bạn đã sẵn sàng chuẩn bị cho thiên thần nhỏ?</h3>
                  <p className="text-slate-400 text-sm leading-relaxed">Tải xuống ngay bộ checklist chuẩn bị đồ đi sinh đầy đủ & chi tiết nhất hoàn toàn miễn phí từ ngoanxinhyeu.</p>
                  <button className="px-8 py-4 bg-white text-slate-900 text-xs font-extrabold uppercase tracking-widest rounded-2xl hover:bg-emerald-500 hover:text-white transition-all shadow-xl active:scale-95">Tải về ngay</button>
                </div>
